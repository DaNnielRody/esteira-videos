...
class AssetPolish:
    ...
    def open_book(self):
        ...
        return book
class OQueCriouOChatGPTScene(AssetPolish, TransparentGPTLogo, Director, VisualScene, MovingCameraScene):
    def construct(self):
        self.begin()
        lead=self.label("O que criou",52,pos=[-3.0,1.8,0])
        article=self.label("o",40,pos=[-5.3,-0.25,0])
        keyword=self.label("ChatGPT",108,self.cyan,[0.3,-0.3,0]).set_width(9.4)
        punctuation=self.label("?",104,self.yellow,[5.6,-0.3,0])
        self.at(0.75,Write(lead),duration=1.15)
        self.at(2.1,Write(article),Write(keyword),duration=1.65)
        self.at(4.0,Write(punctuation),duration=0.65)
        emblem=self.gpt_logo(3.5).move_to([0,0.3,0])
        self.at(5.71,FadeOut(lead,shift=UP),FadeOut(article,shift=LEFT),FadeOut(punctuation,shift=RIGHT),keyword.animate.set_width(4.4).move_to([0,-2.1,0]),GrowFromCenter(emblem),duration=1.5)
        chat=Group(emblem,keyword)
        transformer=self.network(4.7)
        tokens=VGroup(*[self.card(w,self.cyan,size=48) for w in ["to","ken","s"]]).arrange(RIGHT,buff=0.2).set_width(7.5)
        gpu=self.gpu().set_width(6.5)
        ram=VGroup(Rectangle(width=4.8,height=1.5,color=self.cyan,fill_color="#204134",fill_opacity=1),*[Square(0.7,color=self.paper,fill_color="#172338",fill_opacity=1).move_to([x,0,0]) for x in [-1.5,0,1.5]]).set_width(7.5)
        harness=self.assembled_tools().set_height(3.6).move_to([0,-0.3,0])
        label=self.label("Transformer",48,self.yellow,[0,2.4,0])
        # One hero at a time. Receding parts stay in known assembly slots.
        self.at(9.13,FadeOut(emblem,scale=0.08),Create(transformer),FadeOut(keyword,shift=UP*0.3),Write(label),self.camera.frame.animate.move_to([0,0.7,0]).set_width(9),duration=1.5)
        self.at(12,transformer.animate.set_width(2.2).move_to([-4.8,6.4,0]).set_opacity(0.35),FadeOut(label),FadeIn(tokens,shift=RIGHT*3),self.camera.frame.animate.move_to(ORIGIN).set_width(10),duration=1.1)
        self.at(13.55,tokens.animate.set_width(2.8).move_to([-1.5,6.4,0]).set_opacity(0.35),FadeIn(gpu,shift=DOWN*2),self.camera.frame.animate.set_width(10.5),duration=1.3)
        chat=Group(self.gpt_logo(3.5),self.label("ChatGPT",36,pos=[0,-2.3,0]))
        chat.set_width(2.8).move_to([0,-1.2,0])
        self.at(15.23,gpu.animate.set_width(2.8).move_to([2,6.4,0]).set_opacity(0.35),FadeIn(chat),duration=1.3)
        self.at(17.05,FadeIn(ram,shift=LEFT*2),self.camera.frame.animate.set_width(11),FadeOut(chat,shift=DOWN),duration=1)
        self.remove(chat)
        self.at(18.33,ram.animate.set_width(2.4).move_to([5,6.4,0]).set_opacity(0.35),FadeIn(harness,shift=UP),self.camera.frame.animate.move_to([0,-0.5,0]).set_width(9),duration=1.7)
        hlabel=self.label("Harness",40,self.paper,[0,-2.6,0])
        self.play(FadeIn(hlabel),run_time=0.7)
        self.at(21.89,FadeOut(hlabel),*[obj.animate.set_opacity(1).set_y(1.4) for obj in [transformer,tokens,gpu,ram]],harness.animate.set_width(3).move_to([0,-0.5,0]),self.camera.frame.animate.move_to(ORIGIN).set_width(14.22),duration=1.8)
        # CONVERGENCE: components assemble; no cross-concept cables.
        housing=RoundedRectangle(width=11.6,height=4.8,corner_radius=0.22,color=self.cyan,stroke_width=1.5,fill_color="#142337",fill_opacity=0.12).set_z_index(-3)
        self.at(26.45,tokens.animate.set_width(2.7).move_to([-3,-1.15,0]),harness.animate.set_width(2.6).move_to([2.7,-1.15,0]),duration=2)
        self.at(29,transformer.animate.set_width(2.3).move_to([-3,0.8,0]),duration=1.3)
        self.at(31,gpu.animate.set_width(2.1).move_to([1.4,1.15,0]),duration=1.3)
        self.at(33,ram.animate.set_width(2.0).move_to([4,1.15,0]),duration=1.3)
        self.at(35,Create(housing),duration=1.2)
        assembly=Group(transformer,tokens,gpu,ram,harness,housing)
        self.at(36.59,assembly.animate.scale(0.06).move_to([0,-0.4,0]),duration=0.6)
        self.remove(assembly,*assembly)
        # A fresh result actor prevents an exited logo from being reintroduced.
        system_logo=self.gpt_logo(4.8).move_to([0,-0.2,0])
        system_name=self.label("ChatGPT",38,self.ink,[0,-2.7,0])
        result=Group(system_logo,system_name)
        self.play(FadeIn(result,scale=0.8),run_time=0.25)
        system=self.label("um sistema",58,self.green,[0,3,0])
        self.at(37.53,Write(system),duration=1.5)
        self.at(39.55,self.camera.frame.animate.set_width(14.22).move_to([0,-0.25,0]),duration=2.5)
        invite=self.label("VAMOS LÁ",72,self.ink,[0,0,0])
        self.at(44,Succession(AnimationGroup(FadeOut(result,scale=0.8),FadeOut(system)),Write(invite)),duration=1.0)
        # Match the vertical stem of L to the spine; the other letters retire locally.
        stem=invite[5]
        axis=Line([stem.get_left()[0]+0.02,stem.get_bottom()[1],0],[stem.get_left()[0]+0.02,stem.get_top()[1],0],color=self.ink,stroke_width=3)
        book=self.open_book().set_width(7).move_to([-1.6,0,0])
        self.add(axis)
        self.at(45.05,FadeOut(invite,shift=UP*0.12),Transform(axis,book[2]),duration=0.7)
        opened=book.copy()
        hinge=book[2].get_x()
        book.apply_function(lambda p:np.array([hinge+(p[0]-hinge)*0.035,p[1],p[2]]))
        self.remove(axis);self.add(book)
        child=self.person().scale(1.2).move_to([5,-0.5,0])
        self.at(45.8,Transform(book,opened),FadeIn(child,shift=LEFT*0.25),
                self.camera.frame.animate.set_width(12.8).move_to([0.3,0.15,0]),duration=1.6)
        # PERSIST: both actors cross the edit in exactly this resting geometry.
        self.finish(48.75)

    def assembled_tools(self):
        # A control enclosure with ports and routed tools: never another network.
        frame=RoundedRectangle(width=4.8,height=3.5,corner_radius=0.2,color=self.green,stroke_width=3)
        rail=Line([-1.7,1.1,0],[-1.7,-1.1,0],color=self.green,stroke_width=3)
        slots=VGroup(*[RoundedRectangle(width=0.65,height=0.34,corner_radius=0.04,color=self.green,fill_color="#193D36",fill_opacity=1).move_to([-1.7,y,0]) for y in [1,0,-1]])
        tools=VGroup(self.card("{ }",self.cyan,size=27),self.browser("",width=1.2,height=0.8),self.paper_page("",width=0.75,height=0.8)).arrange(DOWN,buff=0.25).move_to([0.9,0,0])
        routes=VGroup(*[VMobject(color=self.green,stroke_width=2).set_points_as_corners([slot.get_right(),[0,slot.get_y(),0],[0,tool.get_y(),0],tool.get_left()]) for slot,tool in zip(slots,tools)])
        return self.actor(VGroup(frame,rail,slots,routes,tools))
