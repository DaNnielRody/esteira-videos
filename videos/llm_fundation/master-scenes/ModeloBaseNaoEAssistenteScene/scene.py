...
class ModeloBaseNaoEAssistenteScene(Director, VisualScene, MovingCameraScene):
    def construct(self):
        self.begin()
        top=VGroup(Arc(radius=2,start_angle=0,angle=PI,color=self.paper,stroke_width=5),Line(LEFT*2,RIGHT*2,color=self.paper,stroke_width=5),*[Line([x,y,0],[x+0.12,y+0.06,0],color=self.paper,stroke_width=2) for x,y in [(-1,0.6),(0,1.3),(0.8,0.8)]]).move_to([-2.4,2.2,0])
        bottom=Arc(radius=1.9,start_angle=PI,angle=PI,color=self.paper,stroke_width=5).stretch(0.25,1).move_to([-2.4,-2,0])
        bridge=self.machine().set_width(4.8).move_to([0,-0.3,0])
        bridge_label=self.label("modelo-base",62,self.yellow,[0,2.5,0])
        self.add(bridge,bridge_label)
        self.play(bridge.animate.set_width(3.8).move_to([-2.4,0,0]),bridge_label.animate.set_width(4).move_to([-2.4,2.5,0]),run_time=1.5)
        ingredients=VGroup(*[self.card(w,c,size=31,width=4.3) for w,c in [("Texto",self.cyan),("Parâmetros",self.yellow),("GPU",self.purple)]])
        for i,part in enumerate(ingredients):part.move_to([-12 if i%2==0 else 12,0.7-i*0.75,0])
        self.add(ingredients)
        self.at(4.84,FadeOut(bridge),FadeOut(bridge_label),FadeIn(top),FadeIn(bottom),LaggedStart(*[part.animate.move_to([-2.4,0.7-i*0.75,0]) for i,part in enumerate(ingredients)],lag_ratio=0.23),bottom.animate.shift(UP*0.3),duration=2.1)
        # The ingredients assemble the imagined JARVIS interface, then the narration rejects the shortcut.
        stem=VMobject(color=self.cyan,stroke_width=3).set_points_as_corners([[-0.2,-0.1,0],[0.6,-0.1,0],[0.6,0.8,0],[1.6,0.8,0]])
        structure=self.jarvis_interface().move_to([3.2,0.8,0])
        self.at(7.1,Create(stem),duration=0.6)
        self.at(7.8,LaggedStart(Create(structure[0]),Create(structure[1]),Create(structure[2]),Write(structure[3]),lag_ratio=0.18),duration=0.8)
        self.remove(*structure);self.add(structure)
        structure[1].add_updater(lambda rings,dt:rings.rotate(0.38*dt,about_point=structure[0].get_center()))
        structure[2].add_updater(lambda rings,dt:rings.rotate(-0.24*dt,about_point=structure[0].get_center()))
        candidate=self.card("assistente?",self.paper,size=27).move_to([3.2,-1.35,0])
        self.play(FadeIn(candidate,shift=UP*0.2),run_time=0.3)
        mismatch=self.label("ainda não",30,self.pink,[3.2,-1.35,0])
        self.until(9.62)
        structure.clear_updaters(recursive=True)
        self.at(9.62,FadeTransform(candidate,mismatch),structure[1].animate.set_stroke(opacity=0.4),structure[2].animate.set_stroke(opacity=0.4),duration=0.7)
        self.play(Indicate(structure[0],color=self.yellow,scale_factor=1.02),run_time=0.6)
        machine=self.machine().set_width(4.5).move_to([0,-1.7,0])
        self.at(12.6,FadeOut(top,shift=UP),FadeOut(bottom,shift=DOWN),FadeOut(ingredients),FadeOut(structure),FadeOut(stem),FadeOut(mismatch),duration=1.5)
        text=self.label("Era uma vez",78,self.cyan,[0,2.5,0]);completion=self.label("um cachorro…",62,self.green,[0,1.1,0])
        self.at(15.5,Write(text),duration=1.2)
        self.at(18.3,Write(completion),duration=1.5)
        request=self.label("Me ajude a programar",62,self.cyan,[0,2.5,0])
        self.at(24,FadeTransform(text,request),FadeOut(completion),duration=1.8)
        # The request becomes context for continuation; intention is not another icon.
        continuation=self.label("Me ajude a escrever…",45,self.paper,[0,1.2,0])
        self.at(27.66,Write(continuation),duration=1.7)
        self.at(30.2,continuation.animate.set_color(self.pink),duration=1.3)
        self.at(32.4,FadeOut(request,shift=UP*0.4),FadeOut(continuation,shift=UP*0.4),GrowFromCenter(machine.move_to(ORIGIN).set_width(6.0)),duration=1.5)
        title=self.label("pós-treinamento",52,self.green,[0,3.3,0])
        self.at(35.8,Write(title),duration=1.8)
        self.finish(38.833333333)


    def jarvis_interface(self):
        # Vector HUD inspired by the supplied concentric cyan reference.
        # The label stays upright; only the segmented rings rotate.
        core=Circle(radius=0.92,color=self.cyan,stroke_width=3)
        inner=VGroup(
            *[Arc(radius=1.13,start_angle=i*TAU/36,angle=TAU/54,color=self.cyan,stroke_width=3) for i in range(36)],
            *[Line(1.27*np.array([np.cos(a),np.sin(a),0]),1.34*np.array([np.cos(a),np.sin(a),0]),color=self.cyan,stroke_width=1.2) for a in np.linspace(0,TAU,64,endpoint=False)],
        )
        outer=VGroup(
            Circle(radius=1.62,color=self.cyan,stroke_width=0.8,stroke_opacity=0.45),
            *[Arc(radius=1.62,start_angle=a,angle=length,color=self.cyan,stroke_width=4) for a,length in [(0.1,0.65),(1.35,0.27),(2.2,0.55),(3.5,0.95),(5.05,0.8)]],
            *[Arc(radius=1.43,start_angle=a,angle=0.95,color=self.cyan,stroke_width=1.7) for a in [0.7,2.85,4.8]],
        )
        title=self.label("J.A.R.V.I.S",25,self.cyan).set_width(1.48)
        return self.actor(VGroup(core,inner,outer,title))
