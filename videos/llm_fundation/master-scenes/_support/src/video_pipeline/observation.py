...
class FrameObservation:
    ...
    instant_seconds: float | None = None
...
class SceneObserver:
    ...
    def observe(
        ...
        *,
        ...
        the verdict was taken from.  ``sample_times`` adds exact logical
        checkpoints to the regular storyboard grid, so a short-lived readable
        state is judged from the frame that actually shows that state.
        """
        ...
        requested_times = _normalise_sample_times(sample_times, duration)
        rate = max(self.samples / duration, 1.0 / duration)
        frame_rate = _video_frame_rate(source) if requested_times else None
        selected_indices = _sample_frame_indices(
            duration,
            self.samples,
            frame_rate,
            requested_times,
        ...
        if selected_indices is None:
            # Sample on a fixed grid so the storyboard is reproducible for one
            # video when its frame rate cannot be read.
            video_filter = f"select=gte(t\\,selected_n*{1 / rate:.9f}),showinfo"
            frame_limit = self.samples
        else:
            video_filter = _frame_selection_filter(selected_indices)
            frame_limit = len(selected_indices)
        argv = [
            "ffmpeg",
            "-v",
            "info",
        ...
        instants = [
            ...
            for value in re.findall(
                r"\bpts_time:([-+0-9.eE]+)", str(getattr(extraction, "stderr", ""))
            )
        ]
        observations = analyze_frames(frames)
        if len(instants) >= len(observations):
            observations = [
                replace(frame, instant_seconds=instants[index])
                for index, frame in enumerate(observations)
            ]
        return ObservationResult.success(observations)


def _normalise_sample_times(times: Sequence[float], duration: float) -> tuple[float, ...]:
    """Validate and de-duplicate logical times requested by the pipeline."""
    ...
    for value in times:
        ...
        if instant <= duration + 1e-6:
            normalised.add(min(instant, duration))
...
def _sample_frame_indices(
    ...
    """Return source frame indices for the grid plus logical checkpoints."""

    if frame_rate is None or not math.isfinite(frame_rate) or frame_rate <= 0.0:
        return None
    last_index = max(0, math.ceil(duration * frame_rate) - 1)
    grid = {
        max(0, min(last_index, math.ceil(index * duration / samples * frame_rate - 1e-9)))
        for index in range(samples)
    }
    grid.update(
        max(0, min(last_index, int(round(instant * frame_rate))))
        for instant in requested_times
    )
    return tuple(sorted(grid))


def _frame_selection_filter(indices: Sequence[int]) -> str:
    """Build an ffmpeg select expression for deterministic source frames."""

    expression = "+".join(f"eq(n\\,{index})" for index in indices)
    return f"select={expression},showinfo"


def _video_frame_rate(path: Path) -> float | None:
    """Read the encoded video rate used to map logical times to frame indices."""
