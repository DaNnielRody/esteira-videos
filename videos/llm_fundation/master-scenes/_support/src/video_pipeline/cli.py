...
from video_pipeline.provider import AuthoredSourceProvider, LLMProvider
...
def _render_video(
    ...
    *,
    ...
    if options.authored_sources is not None:
        provider = AuthoredSourceProvider(options.authored_sources)
...
def _serve_web(options: argparse.Namespace) -> int:
    ...
    if options.authored_sources is not None:
        source_provider = AuthoredSourceProvider(options.authored_sources)

        def authored_pipeline(run_id: str) -> VideoPipeline:
            return VideoPipeline(provider=source_provider, id_factory=lambda: run_id)

        pipeline_factory = authored_pipeline
    try:
        with WebService(
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    render.add_argument("--new-run", action="store_true")
    render.add_argument("--scene")
    render.add_argument("--base-run")
    render.add_argument("--correction")
    render.add_argument("--authored-sources", type=Path)
    ...
    web.add_argument("--authored-sources", type=Path)
