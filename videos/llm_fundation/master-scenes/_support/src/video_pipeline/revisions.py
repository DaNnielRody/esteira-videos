...
_MUTATION_LOCK_NAME = ".revision-store.lock"
...
class RevisionIndex:
    ...
    def from_document(cls, document: Mapping[str, object]) -> RevisionIndex:
        ...
        if set(document) != {"schema_version", "current_revision_id"}:
            raise RequestValidationError("revision index fields are invalid")
        value = document["current_revision_id"]
        if value is not None and not isinstance(value, str):
            raise RequestValidationError("current_revision_id must be a revision id or null")
...
class WorkingDraft:
    ...
    def retry_run_id(self, new_job_id: str) -> str:
        """Derive a bounded, injective run ID from the retry job ID."""

        _validate_id(new_job_id, "new_job_id")
        return f"run-{new_job_id}"
...
class RevisionStore:
    ...
    def publish_terminal(
        ...
        *,
        ...
        _validate_store_project_id(project_id, self.project_root)
        ...
        with self._mutation_lock():
            ...
            if existing is not None:
                if _same_terminal_payload(
                    ...
                    self._repair_missing_index_locked()
                    return existing
                raise StateConflictError(
                    f"terminal job conflict for duplicate job {job_id}"
                )

            # A job's lineage is captured once, when its working draft is
            # created.  The checkout pointer may move while a render runs;
            # consulting it here would attach the terminal result to an
            # unrelated revision selected after the job started.
            draft = self._working_draft(job_id)
            if draft is not None:
                parent_revision_id = draft.parent_revision_id
            else:
                current = self._current_revision()
                parent_revision_id = current.revision_id if current is not None else None
    ...
    def checkout(self, revision_id: str) -> RevisionManifest:
        ...
        with self._mutation_lock():
            ...
            if manifest.status != "success":
                raise StateConflictError("checkout requires a successful revision")
            self._write_index(RevisionIndex(current_revision_id=manifest.revision_id))
            return manifest
    ...
    def start_working(
        ...
        *,
        ...
        if not status or not isinstance(status, str):
            raise RequestValidationError("working status must be a non-empty string")
        if status in _TERMINAL_DRAFT_STATES:
            raise RequestValidationError("start_working requires a non-terminal status")
        hashes = _package_hashes(base_package_hashes)
        message_list = _string_list(messages, "messages")
        asset_list = _safe_id_list(asset_ids, "asset_ids")
        _validate_optional_text(correction, "correction", self.project_root)
        ...
        if (scene_id is None) != (base_run_id is None):
            raise RequestValidationError("scene_id and base_run_id must be supplied together")
        with self._mutation_lock():
            ...
            if path.exists():
                ...
                if existing.to_document() == draft.to_document():
                    return existing
                raise StateConflictError(
                    f"working job conflict for duplicate job {job_id}"
                )
            _write_json_create_only(path, draft.to_document())
            return draft

    def recover_interrupted(self) -> list[WorkingDraft]:
        """Mark persisted drafts interrupted with incremental atomic updates.

        Each draft replacement is atomic; if a later replacement fails, the
        remaining drafts are retried by the next recovery.
        """

        with self._mutation_lock():
            self._repair_missing_index_locked()
            self._reconcile_terminal_drafts_locked()
            return self._recover_interrupted_locked()

    def _reconcile_terminal_drafts_locked(self) -> set[str]:
        """Close drafts whose terminal manifest was committed before a crash."""

        reconciled: set[str] = set()
        for path in self._working_paths():
            draft = WorkingDraft.from_document(_load_document(path))
            if draft.status in _TERMINAL_DRAFT_STATES - {"interrupted"}:
                continue
            manifest = self._revision_for_job(draft.job_id)
            if manifest is None or manifest.status not in {"success", "failure"}:
                continue
            self._terminalize_working_draft(manifest)
            reconciled.add(draft.job_id)
        return reconciled

    def _recover_interrupted_locked(self) -> list[WorkingDraft]:
        """Recover drafts while the caller owns the mutation lock."""
    ...
    def retry(
        ...
        *,
        ...
        """Create a queued retry while preserving its interrupted source.

        A normal retry gets a fresh run identifier, retaining the historical
        behavior.  A full render that crashed after the canonical run was
        opened can explicitly continue that run with ``reuse_run=True``.
        The source draft is never modified in either mode.
        """

        _validate_id(job_id, "job_id")
        _validate_id(new_job_id, "new_job_id")
        if not isinstance(reuse_run, bool):
            raise RequestValidationError("reuse_run must be a boolean")
        with self._mutation_lock():
            source_path = self.working_root / f"{job_id}.json"
            if not source_path.is_file() or source_path.is_symlink():
                raise NotFoundError(f"unknown working job {job_id}")
            ...
            if target_path.exists():
                raise StateConflictError(
                    f"working job conflict for duplicate job {new_job_id}"
                )
            retry_run_id = source.run_id if reuse_run else source.retry_run_id(new_job_id)
            if not reuse_run:
                for path in self._working_paths():
                    existing = WorkingDraft.from_document(_load_document(path))
                    if existing.run_id == retry_run_id:
    ...
    def _repair_missing_index_locked(self) -> str | None:
        """Repair only an absent, unambiguous first checkout pointer."""

        index_path = self.ui_root / "index.json"
        try:
        ...
        if len(paths) != 1 or paths[0].stem != "v001":
            return None
        manifest = self._load_revision("v001")
        self._write_index(RevisionIndex(current_revision_id=manifest.revision_id))
        return manifest.revision_id

    @contextmanager
    def _mutation_lock(self) -> Iterator[None]:
        """Serialize revision mutations across threads and processes."""

        lock_path = self.ui_root / _MUTATION_LOCK_NAME
        self._preflight_lock(lock_path)
        descriptor = os.open(
            lock_path,
    ...
    def _preflight_lock(self, path: Path) -> None:
        """Reject a lock path that could redirect writes outside the project."""
        ...
        try:
            ...
            raise ValueError("cannot inspect revision store lock") from exc
        if stat.S_ISLNK(mode) or not stat.S_ISREG(mode):
            raise ValueError("revision store lock must be a real regular file")

    def _working_draft(self, job_id: str) -> WorkingDraft | None:
        """Load a job draft when present, preserving its captured lineage."""
        ...
        try:
            ...
            raise ValueError(f"cannot inspect working draft {job_id}") from exc
        if stat.S_ISLNK(mode) or not stat.S_ISREG(mode):
            raise ValueError(f"working draft {job_id} must be a real file")
        return WorkingDraft.from_document(_load_document(path))
    ...
    def _load_revision(self, revision_id: str) -> RevisionManifest:
        ...
        if manifest.project_id != self.project_root.name:
            raise ValueError(f"revision project identity mismatch for {revision_id}")
        return manifest

    def _revision_for_job(self, job_id: str) -> RevisionManifest | None:
        for path in self._revision_paths():
            manifest = self._load_revision(path.stem)
...
def _validate_store_project_id(value: str, project_root: Path) -> None:
    _validate_id(value, "project_id")
    if value != project_root.name:
        raise ValueError("project_id must match project root")
