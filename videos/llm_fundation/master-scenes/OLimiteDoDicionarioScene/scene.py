...
class AssetPolish:
    ...
    def open_book(self):
        ...
        return book
class OLimiteDoDicionarioScene(AssetPolish, Director, VisualScene, MovingCameraScene):
    def construct(self):
        self.begin()
        self.camera.frame.set_width(12.8).move_to([0.3,0.15,0])
        book=self.open_book().set_width(7).move_to([-1.6,0,0]);self.add(book)
        child=self.person().scale(1.2).move_to([5,-0.5,0])
        self.add(child)
        self.at(2.1,self.look_at(child,book),duration=2)
        self.at(5.24,self.confused(child),duration=0.8)
        self.play(self.blink(child),run_time=0.35)
        # Four original glyphs emerge over the full listening interval.
        first_word=self.label("CASA",30,self.cyan,[2.9,2.0,0])
        templates=[letter.copy() for letter in first_word]
        origin=child[0].get_top()+LEFT*0.08
        def assemble_letters(word,alpha):
            elapsed=alpha*7.8
            settle=1-smooth(np.clip((elapsed-6.3)/1.5,0,1))
            for i,(letter,template) in enumerate(zip(word,templates)):
                u=smooth(np.clip((elapsed-i*1.8)/1.1,0,1))
                goal=template.get_center()
                c1=origin+UP*1.05+RIGHT*0.12
                c2=goal+UP*0.7+LEFT*0.3
                position=(1-u)**3*origin+3*(1-u)**2*u*c1+3*(1-u)*u*u*c2+u**3*goal
                # A small circular drift while waiting for the next letter,
                # then settle exactly into the word before the dictionary search.
                if u>=1:
                    phase=elapsed*1.4+i*0.6
                    position+=settle*0.07*np.array([np.sin(phase),np.cos(phase),0])
                letter.become(template.copy().scale(max(0.001,u)).rotate(-0.38*(1-u)).move_to(position).set_opacity(u))
        left_rows=list(reversed(list(book[3])[:5]))
        right_rows=list(reversed(list(book[3])[5:]))
        entries=VGroup(*[self.on_rule(self.label(w,30,self.paper),line) for w,line in zip(["casa","gato","carro","cachorro"],left_rows)])
        scanner=left_rows[0].copy().set_stroke(self.cyan,width=4,opacity=1).set_z_index(7)
        self.until(6.8)
        assemble_letters(first_word,0);self.add(first_word)
        self.follow_word(child,first_word)
        # Keep the existing entry-writing and scan cues (9.5 and 12.1 s)
        # while the letters arrive at 6.8, 8.6, 10.4, and 12.2 s.
        self.play(AnimationGroup(
            UpdateFromAlphaFunc(first_word,assemble_letters,run_time=7.8,rate_func=linear),
            Succession(Wait(2.7),Write(entries,run_time=1.8)),
            Succession(Wait(5.3),Create(scanner,run_time=0.8)),
            lag_ratio=0,
        ),run_time=7.8,rate_func=linear)
        child[2].clear_updaters()
        assemble_letters(first_word,1)
        for i,t in enumerate([15.0,16.2,17.3,18.45]):
            word=first_word if i==0 else self.label(["CASA","GATO","CARRO","CACHORRO"][i],30,self.cyan,[2.9,2.0,0])
            self.until(t)
            if i:
                self.play(LaggedStart(*[GrowFromPoint(letter,child[0].get_top()+LEFT*0.08) for letter in word],lag_ratio=0.12),
                          self.look_at(child,word),run_time=0.2)
                self.remove(*word);self.add(word)
            self.play(self.look_at(child,word),self.send_word(child,word),run_time=0.1)
            self.follow_word(child,word)
            self.play(Transform(word,entries[i].copy().set_color(self.cyan)),Transform(scanner,left_rows[i].copy().set_stroke(self.cyan,width=4,opacity=1).set_z_index(7)),run_time=0.45)
            child[2].clear_updaters();self.remove(word)
            self.play(entries[i].animate.set_color(self.green),scanner.animate.set_color(self.green),run_time=0.15)
            self.play(VGroup(*child[:4]).animate(rate_func=there_and_back).shift(DOWN*0.14),run_time=0.15)
            self.checkpoint("dictionary-found-"+str(i))
        incoming=self.label("ornitorrinco",40,self.cyan,[2.9,2.8,0])
        self.at(21,Write(incoming),self.look_at(child,incoming),duration=1.0)
        self.follow_word(child,incoming)
        self.at(22.1,Transform(incoming,self.on_rule(self.text("ornitorrinco",30,self.cyan),right_rows[0])),Transform(scanner,right_rows[0].copy().set_stroke(self.cyan,width=4,opacity=1).set_z_index(7)),duration=0.5)
        for line in right_rows[1:]:
            self.play(Transform(incoming,self.on_rule(self.text("ornitorrinco",30,self.cyan),line)),Transform(scanner,line.copy().set_stroke(self.cyan,width=4,opacity=1).set_z_index(7)),run_time=0.5)
        child[2].clear_updaters()
        self.at(25.0,scanner.animate.set_color(self.pink),book[1].animate.set_stroke(self.pink,width=4),duration=0.25)
        self.play(book[1].animate.set_stroke(self.paper,width=2.3),run_time=0.2)
        self.play(Transform(incoming,self.text("ornitorrinco",40,self.pink).set_width(3.7).move_to([2.9,2.8,0])),run_time=0.6)
        self.play(self.look_at(child,incoming),self.react_confused(child),run_time=0.35)
        neutral_mouth=Line(child[3].get_left(),child[3].get_right(),color=self.paper,stroke_width=2)
        self.play(Transform(child[3],neutral_mouth),run_time=0.2)
        doubt=self.label("?",32,self.paper).next_to(child[0],UP,buff=0.18)
        self.play(FadeIn(doubt),run_time=0.25)
        self.play(FadeOut(doubt),run_time=0.3)
        self.checkpoint("dictionary-search-exhausted-before-reaction-and-UNK")
        unknown=self.tag("UNK",self.pink,size=48).set_width(3.6).move_to([2.9,2.8,0])
        self.at(29.4,ReplacementTransform(incoming,unknown),self.camera.frame.animate.set_width(9.4).move_to([2.1,1.6,0]),duration=1.6)
        meaning=self.label("desconhecido",32,self.paper).next_to(unknown,UP,buff=0.22)
        self.at(33,FadeIn(meaning),self.look_at(child,unknown),duration=1.2)
        self.at(36,self.camera.frame.animate.set_width(12.8).move_to([0.3,0.15,0]),FadeOut(unknown),FadeOut(meaning),FadeOut(entries),FadeOut(scanner),duration=1.6)
        rare=self.label("palavra rara",40,self.cyan,[2.3,2.3,0])
        self.at(38.0,FadeIn(rare,shift=LEFT*0.3),self.look_at(child,rare),duration=0.8)
        for word,t in [("nome novo",40.02),("variação",41.2),("cringe",42.5)]:
            next_word=self.label(word,40,self.cyan,[2.3,2.3,0])
            animations=[FadeIn(next_word,shift=LEFT*0.3)] if rare is None else [FadeTransform(rare,next_word)]
            self.at(t,*animations,self.look_at(child,next_word),duration=0.85);rare=next_word
        scan=right_rows[0].copy().set_stroke(self.cyan,width=5,opacity=1).set_z_index(7)
        self.follow_word(child,rare)
        self.at(43.5,Transform(rare,self.on_rule(self.text("cringe",34,self.cyan),right_rows[0])),Create(scan),duration=0.6)
        for line in right_rows[1:]:
            self.play(Transform(rare,self.on_rule(self.text("cringe",34,self.cyan),line)),Transform(scan,line.copy().set_stroke(self.cyan,width=5,opacity=1).set_z_index(7)),run_time=0.4)
        child[2].clear_updaters()
        self.play(scan.animate.set_color(self.pink),book[1].animate.set_stroke(self.pink,width=4),run_time=0.25)
        self.play(Transform(rare,self.text("cringe",40,self.pink).move_to([2.3,2.3,0])),run_time=0.4)
        self.play(self.look_at(child,rare),self.react_confused(child),run_time=0.4)
        doubt=self.label("?",32,self.paper).next_to(child[0],UP,buff=0.15)
        self.play(FadeIn(doubt),run_time=0.2)
        failed=self.tag("UNK",self.pink,size=42).set_width(2.1).move_to(rare)
        self.play(ReplacementTransform(rare,failed),FadeOut(doubt),FadeOut(scan),book[1].animate.set_stroke(self.paper,width=2.3),run_time=0.45)
        rare=failed
        self.checkpoint("slang-search-failed-before-UNK")
        self.until(48.8)
        self.play(FadeOut(rare),run_time=0.5)
        # The question is built over the unresolved dictionary, then used as a word.
        # Every cue lands on the spoken words (word-level alignment):
        # 49.7 "e é aí que aparece uma ideia muito massa" -> dictionary dims,
        # 54.21 "precisa decorar" -> lead, 55.14-56.4 "cada palavra inteira" -> key.
        lead=self.label("PRECISA DECORAR",38,self.paper,[0,2.0,0])
        key=self.label("PALAVRA INTEIRA",64,self.yellow,[0,0.1,0]).set_width(9.7)
        question=self.label("?",72,self.yellow,[5.6,0.1,0])
        self.at(49.70,FadeOut(child),duration=1.0)
        self.at(50.90,book.animate.set_opacity(0.25),duration=1.4)
        self.at(52.59,Write(lead),duration=1.3)
        self.at(54.21,Write(key),Write(question),duration=1.3)
        slot=SurroundingRectangle(key,buff=0.23,color=self.cyan,corner_radius=0.07)
        self.at(55.55,Create(slot),FadeOut(lead,shift=UP),FadeOut(question,shift=RIGHT),duration=0.7)
        self.play(FadeOut(slot),run_time=0.15)
        word=self.label("INFELIZMENTE",80).set_width(11.1)
        # The payoff lands right after "cada palavra inteira?" (56.40): the
        # word itself becomes the subject. The whole-word slot is too small —
        # lexical tension prepares the approved cuts in the next take.
        self.at(56.40,Succession(FadeOut(key,scale=0.9),GrowFromCenter(word)),book.animate.set_width(5).move_to([0,-2.5,0]).set_opacity(0.12),self.camera.frame.animate.set_width(14.22).move_to(ORIGIN),duration=0.6)
        self.play(FadeOut(book),run_time=0.25)
        self.wait(0.15)
        self.finish(57.375)

    def follow_word(self,child,word):
        def follow(eyes):
            delta=word.get_center()-child[0].get_center()
            delta=delta/(np.linalg.norm(delta)+1e-8)*0.095
            eyes.move_to(child[0].get_center()+UP*0.065+delta)
        child[2].add_updater(follow)

    def on_rule(self,word,line):
        a,b=line.get_start(),line.get_end()
        if a[0]>b[0]:a,b=b,a
        delta=b-a;angle=np.arctan2(delta[1],delta[0])
        word.scale_to_fit_width(min(word.width,np.linalg.norm(delta)*0.88))
        height=word.height
        normal=np.array([-np.sin(angle),np.cos(angle),0])
        word.rotate(angle).move_to((a+b)/2+normal*(height/2+0.035))
        return word.set_z_index(8)

    def send_word(self,child,target):
        shoulder=child[5][0].get_start()
        delta=target.get_center()-shoulder
        delta/=np.linalg.norm(delta)+1e-8
        # Use the near arm: the gesture must never cross the face.
        return Transform(child[5][0],Line(shoulder,shoulder+delta*0.65,color=self.paper,stroke_width=5))
