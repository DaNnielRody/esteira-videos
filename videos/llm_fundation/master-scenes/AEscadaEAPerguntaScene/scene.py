...
class AEscadaEAPerguntaScene(Director, VisualScene, MovingCameraScene):
    def construct(self):
        self.begin()
        labels=["Texto / Tokens","Aprender","Transformer","Escala","Modelo-base","Pós-treino","Assistente"]
        stairs=VGroup()
        for i,name in enumerate(labels):
            x=i*3.7;y=i*1.35
            front=Polygon([x,0,0],[x+3.7,0,0],[x+3.7,y+1.35,0],[x,y+1.35,0],color=self.cyan,fill_color="#1C3E58",fill_opacity=1,stroke_width=2)
            cap=Polygon([x,y+1.35,0],[x+3.7,y+1.35,0],[x+4.15,y+1.7,0],[x+0.45,y+1.7,0],color=self.cyan,fill_color="#366680",fill_opacity=1,stroke_width=2)
            text=self.label(name,32,self.ink,[x+1.85,y+0.55,0]);text.set_width(min(3.1,text.width))
            stairs.add(VGroup(front,cap,text))
        self.camera.frame.set_width(11.2).move_to([3,2.2,0])
        self.play(FadeIn(stairs),run_time=1.2)
        # Reuse token glyphs and model cells as small callbacks on the treads.
        callbacks=VGroup(self.label("IN | FELIZ | MENTE",15,self.yellow,[1.85,1.0,0]),self.label("→ próximo token",15,self.yellow,[5.55,2.35,0]))
        self.play(LaggedStart(*[FadeIn(label,shift=UP*0.06) for label in callbacks],lag_ratio=0.12),run_time=0.8)
        self.at(2.1,self.camera.frame.animate.shift(RIGHT*0.4),duration=1.8)
        marker=Dot([1.4,1.7,0],radius=0.15,color=self.yellow);self.add(marker)
        times=[5.84,6.68,8,9.4,11.24,13.46,15.54]
        for i,t in enumerate(times):
            duration=min((times[i+1]-t-0.1) if i<6 else 1.3,1.6)
            self.at(t,self.camera.frame.animate.move_to([i*3.7+2.5,i*1.35+2.1,0]),marker.animate.move_to([i*3.7+1.85,i*1.35+1.68,0]),duration=duration)
        child=self.person().set_height(2.5)
        child.move_to([24.05,9.8+child.height/2,0])
        here=self.label("VOCÊ ESTÁ AQUI",38,self.yellow,[21.8,13.2,0])
        pointer=Arrow(here.get_bottom(),child.get_top()+UP*0.12,buff=0.12,color=self.yellow,stroke_width=3)
        self.at(17,FadeOut(marker),FadeIn(child,shift=UP*0.12),self.look_at(child,stairs[3]),duration=0.8)
        self.at(17.8,self.camera.frame.animate.move_to([13.1,6.8,0]).set_width(30.0),FadeIn(here),GrowArrow(pointer),duration=4.0)
        self.at(25.2,self.look_at(child,VectorizedPoint([27,16,0])),duration=0.6)
        question=self.label("Como isso\né possível?",65,self.ink,[24.5,15.0,0])
        self.at(26,Succession(AnimationGroup(FadeOut(here),FadeOut(pointer)),Write(question)),self.camera.frame.animate.move_to([22,13.1,0]).set_width(18),duration=1.4)
        self.at(28,FadeOut(stairs),FadeOut(callbacks),FadeOut(child),question.animate.move_to([0,2,0]),self.camera.frame.animate.move_to(ORIGIN).set_width(14.22),duration=1.5)
        # Rapid callbacks of everything already learned, riding the narration:
        # "pré-treinamento... uma coisa só... o próximo token" (27.3–36.7),
        # "termina em algo que consegue..." (42.0+), then the five capabilities
        # are named one by one (43.6–48.3). Match cuts and morphs only — no
        # re-explanation, and the pile is never the standing protagonist.
        phrase=self.label("cachorro",80,self.cyan,[0,0.8,0])
        model=self.machine().set_width(5.3).move_to([0,-0.9,0])
        # The word arrives already wearing its token rim — recognition, not re-demo.
        token_rim=SurroundingRectangle(phrase,buff=0.22,color=self.cyan,corner_radius=0.08)
        self.at(30,Succession(AnimationGroup(FadeOut(question,shift=UP*0.3),FadeIn(phrase,shift=UP*0.3)),Create(token_rim)),duration=1.2)
        self.play(FadeOut(token_rim),run_time=0.35)
        vector=self.label("[0.18, −0.72, 0.42, …]",50,self.cyan,[0,0.8,0]).set_width(8.0)
        self.at(31.8,ReplacementTransform(phrase,vector),duration=0.8)
        self.at(33.0,Succession(vector.animate.set_width(3.4).move_to([-4.5,0.2,0]),GrowFromCenter(model)),duration=1.0)
        self.at(34.1,vector.animate.scale(0.04).move_to(model.get_left()),duration=0.65)
        self.remove(vector)
        self.play(LaggedStart(*[ShowPassingFlash(plate[0].copy().set_fill(opacity=0).set_stroke(self.green,width=4)) for plate in model],lag_ratio=0.15),run_time=0.9)
        token=self.card("próximo token",self.green,size=34).set_width(3.4).move_to([4.75,0.2,0])
        self.at(35.94,GrowFromPoint(token,model.get_right()),duration=0.8)
        context=self.label("O cachorro correu atrás da",40,self.ink,[0,1.2,0])
        if context.width>11:context.scale_to_fit_width(11)
        answer=self.label("bola",60,self.green,[0,-0.1,0])
        self.at(37.1,FadeOut(token),model.animate.set_width(3.4).move_to([0,-1.8,0]),FadeIn(context),duration=1.0)
        self.at(38.5,ShowPassingFlash(model[-1][0].copy().set_fill(opacity=0).set_stroke(self.green,width=4)),duration=0.6)
        self.at(39.2,GrowFromPoint(answer,model.get_top()),duration=0.6)
        self.at(40.0,answer.animate.next_to(context,DOWN,buff=0.2),duration=0.7)
        # PERSIST: functional model in the same geometry as the next take.
        self.at(41.4,FadeOut(context),FadeOut(answer),duration=1.4)
        task=self.label("cachorro → dog",48,self.green,[0,0.8,0])
        self.at(43.62,GrowFromPoint(task,model.get_top()),
                ShowPassingFlash(model[-1][0].copy().set_fill(opacity=0).set_stroke(self.green,width=4)),duration=0.8)
        code=self.label("somar(a,b)",48,self.green,[0,0.8,0])
        self.at(44.8,Transform(task,code),duration=0.65)
        # A brief summary callback uses the same processing stage and exits.
        summary=self.label("Chuva → alagamento",32,self.cyan,[0,2.3,0])
        self.at(45.86,FadeIn(summary,shift=DOWN*0.15),duration=0.3)
        self.play(ShowPassingFlash(model[-1][0].copy().set_fill(opacity=0).set_stroke(self.green,width=4)),run_time=0.25)
        self.play(FadeOut(summary,shift=UP*0.15),run_time=0.2)
        concrete=self.label("2 + 2",48,self.green,[0,0.8,0])
        question=self.label("Quanto é?",32,self.cyan,[0,2.3,0])
        self.at(46.98,Transform(task,concrete),FadeIn(question,shift=DOWN*0.15),duration=0.55)
        self.play(LaggedStart(*[ShowPassingFlash(plate[0].copy().set_fill(opacity=0).set_stroke(self.green,width=4)) for plate in model],lag_ratio=0.15),run_time=0.5)
        result=self.label("2 + 2 → 4",48,self.green,[0,0.8,0])
        self.at(48.16,Transform(task,result),FadeOut(question,shift=UP*0.15),duration=0.65)
        # The completed expression and model match the next scene.
        self.finish(49.166666667)
