...
def normalize_scene(
    ...
    *,
    ...
    The signed delta is ``observed - target``. Every accepted candidate must
    share the composition media profile, including its timebase. Deltas within
    the correction limit use an explicit trim or cloned-frame pad;
...
def _result_after_validation(
    ...
    # Narration boundaries may fall between video frames. Re-encoding cannot
    # represent them more precisely than one frame at the requested FPS.
    elif abs(validated_duration - target_duration_seconds) > 1.0 / target_fps + 1e-6:
        reasons.append(
