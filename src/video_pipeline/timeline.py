...
from video_pipeline.reference_catalog import REFERENCE_TOPICS, ReferenceTopic
...
_METADATA = re.compile(r"^@(start|end|objective|capabilities|topics):[ \t]*(.*?)[ \t]*$")
...
def _parse_sections(script: str) -> tuple[tuple[ExplicitScene, ...], bool]:
    ...
    for section_index, (heading_index, title) in enumerate(heading_indexes):
        ...
        topics_by_name: dict[str, ReferenceTopic] = {topic: topic for topic in REFERENCE_TOPICS}
        try:
            topics = tuple(
                topics_by_name[item.strip()] for item in topic_values.split(",")
            ) if topic_values else ()
        except KeyError as exc:
            raise ValueError("scene topics include an unknown reference topic") from exc
        if len(topics) > 4 or len(topics) != len(set(topics)):
            raise ValueError("topics must contain at most four unique entries")
