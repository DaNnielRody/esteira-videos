"""Public contract for proportional fallback timeline initialization."""
...
class EmptySilenceDetector:
    """The fallback path observes that no eligible pauses were available."""
...
def test_init_without_eligible_pauses_uses_proportional_candidate(tmp_path: Path) -> None:
    ...
    assert "pause" in limitation
    assert "unavailable" in limitation
    assert "proportional" in limitation
    assert "approximate" in limitation
    assert "unverified" in limitation
